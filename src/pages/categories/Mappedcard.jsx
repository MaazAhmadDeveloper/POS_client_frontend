...
function Mappedcard( {productObj, getAllCategories, setEditProduct, setPopModal, getAllProducts, categoryClickHandle} ) {
    ...
    const handlerDelete = async (idCategory, imagePath) => {
      ...
      <div onClick={() => categoryClickHandle(productObj.category)} style={{display: "flex"}} >
        ...
        <Modal title="Delete Category" visible={deleteModelPopUp} onCancel={() => setDeleteModelPopUp(false) } footer={false}>
            ...
            <div style={{display: "flex"}}>
                ...
                <Button className='delete-category' onClick={()=>{ handlerDelete(productObj._id, productObj.image ); setDeleteModelPopUp(false)  }}>Cofirm</Button>
            </div>
        </Modal>
  </Card>
  )
